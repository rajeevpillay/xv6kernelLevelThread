...
//lab 3 global declaration for thread and lock
int next_thrd_id=1;
struct spinlock thrd_id_lk;
...
//allocate thread id function, similar to for process
int alloctid()
{
  int tid;
...
// initializating thread and thread memory allocation, similar to allocprocess 
static struct proc*
allocthread(void)
{
  struct proc *p;
...
found:
  ...
  //pagetable section removed because it is being shared with parent
...
//function to create a thread and copy parent. like fork for process
